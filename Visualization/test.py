...
def get_stock_data():
    while True:
        ...
        try:
            response = requests.get(f"http://localhost:5000/data/?stock={stock}")
            print(response.json())
            print("\n\n ****************************************   \n\n")
...
# Thread function for receiving data via socket to retrive real-time closing_price of stocks
def socket_receive_json():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            s.connect(("localhost", 13524))
            while True:
                data = s.recv(1024)
                if not data:
                    break
...
# Thread function for receiving messages via another socket to get trading signals
...
thread4 = threading.Thread(target=socket_receive_message)
